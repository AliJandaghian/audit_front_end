...
import { getAuditSettings, deleteAuditSetting } from "../services/auditSettingService";
...
class AuditSetting extends Component {
    ...
    async componentDidMount() {
        ...
        const auditSettings = data.filter(a=>a.department === '6245e63578dcce0ad767b138')
    ...
    handleAfterSave = (auditSetting) => {
        ...
        if (this.state.auditSettingId === "new") {
          ...
          let oldAuditSetting = auditSettings.find((p) => p._id === auditSetting._id);
  ...
  handleDelete = async (auditSettingId) => {
    ...
    const auditSettings = originalAuditSettings.filter((auditSetting) => auditSetting._id !== auditSettingId);
    this.setState({ auditSettings });
    try {
    ...
    handleSort = (sortColumn) => {
      ...
      getPagedData = () => {
        const { pageSize, currentPage, sortColumn, auditSettings: allAuditSttings } = this.state;
        const sorted = _.orderBy(allAuditSttings, [sortColumn.path], [sortColumn.order]);
    ...
    render() { 
        ...
        <Pagination
          ...
          {this.state.isOpen && (
          <AuditSettingForm
            closeModal={() => this.setState({ isOpen: false, auditSettingId: "new" })}
